...
class AudioPlayer(
  ...
  fun setMediaSources(
    ...
    ref.setMediaSources(sources, startIndex, startPositionMs)
    ref.prepare()
    startUpdating()
    if (autoPlay) {
      ref.play()
  ...
  fun seekToMediaItem(index: Int, seekTime: Double? = null, autoPlay: Boolean? = null) {
    if (index < 0 || index >= ref.mediaItemCount) {
      return
    }

    if (seekTime != null) {
      ref.seekTo(index, (seekTime * 1000L).toLong())
    } else {
  ...
  fun currentStatus(): Map<String, Any?> {
    ...
    return mapOf(
      ...
      "currentMediaItemCount" to ref.mediaItemCount,
