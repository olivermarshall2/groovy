...
class AudioModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    Class(AudioPlayer::class) {
      ...
      Function("replaceQueue") { player: AudioPlayer, sources: List<AudioSource>, startIndex: Int, startPositionSeconds: Double?, playWhenReady: Boolean? ->
        runOnMain {
          if (!player.ref.availableCommands.contains(Player.COMMAND_CHANGE_MEDIA_ITEMS)) {
            return@runOnMain
          }
          ...
          val resolvedPlayWhenReady = playWhenReady ?: true
          if (resolvedPlayWhenReady && !focusAcquired) {
            requestAudioFocus()
          }

          val boundedStartIndex = startIndex.coerceIn(0, mediaSources.lastIndex)
          val startPositionMs = ((startPositionSeconds ?: 0.0) * 1000L).toLong().coerceAtLeast(0L)
          player.setMediaSources(mediaSources, boundedStartIndex, startPositionMs, resolvedPlayWhenReady)
        }
      }

      Function("seekToQueueItem") { player: AudioPlayer, index: Int, startPositionSeconds: Double?, playWhenReady: Boolean? ->
        runOnMain {
          val resolvedPlayWhenReady = playWhenReady ?: player.ref.isPlaying
          if (resolvedPlayWhenReady && !focusAcquired) {
            requestAudioFocus()
          }
          player.seekToMediaItem(index, startPositionSeconds, resolvedPlayWhenReady)
        }
      }
